...
export default function TickerBar({ ticker }: { ticker: TickerData | null }) {
  if (!ticker) {
    return (
      <div className="w-full bg-zinc-900 rounded-xl p-4 text-zinc-500 text-sm mb-5">
        Loading market data…
      </div>
    );
  }
  ...
  return (
    <div className="w-full bg-zinc-900 rounded-xl p-4 grid grid-cols-2 md:grid-cols-4 gap-4 text-sm mb-5">
...
function Stat({ label, value, highlight }: { label: string; value: string | number | undefined; highlight?: "up" | "down" }) {
  return (
    <div>
      ...
      <div className={`font-semibold ${highlight === "up" ? "text-emerald-400" : highlight === "down" ? "text-red-400" : "text-white"}`}>
        {value ?? "—"}
      </div>
    </div>
  );
