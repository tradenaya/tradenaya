...
export default function AllPositionsPage() {
  ...
  if (loading) {
    return <div className="p-6 text-zinc-500">Loading positions & orders…</div>;
  }
  ...
  return (
    <div className="min-h-screen bg-black text-white p-6">
          ...
          <div className="space-y-2">
            {positions.map((pos) => {
              ...
              return (
                <div
                  key={pos.position_id}
                  onClick={() => router.push(`/trade/${pos.symbol}`)}
                  className="bg-zinc-900 rounded-lg p-4 flex items-center justify-between cursor-pointer hover:bg-zinc-800"
                >
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-zinc-500 text-sm">
                      {pos.position_size} @ {pos.avg_entry_price}
                    </span>
                  </div>
                  ...
                  <div className="text-right">
                    ...
                    <div className="text-zinc-600 text-xs">
                      Margin: {pos.position_margin} USDT
                    </div>
                  </div>
                </div>
              );
      ...
      <section>
        ...
        {orders.length === 0 ? (
          <p className="text-zinc-500 text-sm">No open orders.</p>
        ) : (
          <div className="space-y-2">
            {orders.map((order) => (
              <div
              ...
              >
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-zinc-500 text-xs">{order.order_type}</span>
                    <span className="text-zinc-600 text-xs">{order.status}</span>
                  </div>
                  <div className="text-zinc-400 text-sm">
                    {order.quantity} @ {order.price} USDT
                  </div>
                </div>
